...
import styles from './create-doc-modal.module.css';
...
export default function CreateDocModal({ onClose, onCreated, initialType = 'text' }: CreateDocModalProps) {
  ...
  const handleCreate = () => {
    ...
    const doc = createNewDocument(finalName, type, content, selectedTemplate || undefined);
    onCreated();
    navigate({ to: '/editor/$id', params: { id: doc.id } as any });
  };
